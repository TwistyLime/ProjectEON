...
class HazmatChestplate {
    static open(player) {
        let menu = new ChestFormData("large");
        menu.title("§l§5Project Eon | Hazmat Chestplate");
        menu.pattern(
            [
                "xyxxxxxyx",
                "x___xxxxx",
        ...
        for (const item of recipe) {
            menu.button(
                item.slot,
                `§l§5${item.name}`,
                ["§7Part of the Hazmat Chestplate crafting recipe."],
                item.texture,
                item.amount,
                0,
                false
...
export function openHazmatChestplateRecipe(player) {
    HazmatChestplate.open(player);
}
